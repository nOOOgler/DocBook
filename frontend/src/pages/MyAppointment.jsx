...
const MyAppointment = () => {
  ...
  return (
    <div>
      <p className='text-2xl pb-3 mt-12 font-medium text-zinc-700 border-b'>My Appointments </p>
      <div>
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            <div>
              <img className='w-48 bg-indigo-50' src={item.docData.image} alt="" />
            </div>
            <div className='flex-1 text-md text-zinc-600'>
              ...
              <p className='text-xs mt-1'><span className='text-sm text-neutral-700 font-medium' >Date & Time:</span>: {slotDate(item.slotDate)} |  {item.slotTime}</p>
            </div>
            <div></div>
